...
function locate(){
    ...
    pop1 = locator[0];
    pop2 = locator[1];
    arr[pop1][pop2].attr('id','select');
    var inputElement = document.getElementById('select');
    ob = new ttData(pop1, pop2, 0, 0);
    document.getElementById("here").innerHTML = ob.ans + " &#247; " + ob.num1 + " = ";
    var d = new Date();
    startTime = d.getTime();
//    cheat function:
//    document.getElementById("guessInput").value = ob.num2;
}
...
function checkAns() {
    ...
    if (Uans == ob.num2){
        ...
        obList.push(ob);
        arr[pop1][pop2].attr('class','correct');
        if(ttList.length > 0){
        ...
        ob.err++;
        document.getElementById("guessInput").value = "";
        arr[pop1][pop2].attr('class','error');
        $( '#tbl' ).effect( 'shake' );        
    }
}
